...
<p class="titulo">AGREGAR CLIENTES</p>
<form id="form1" name="form1" method="post" action="<?= site_url('clientes/guardar') ?>">
...
<?php endif; ?>
  ...
  <p>
    ...
    <input name="nombreC" type="text" id="nombreC" size="20" maxlength="20" required
        value="<?= esc(old('nombreC')) ?>" onkeyup="this.value=this.value.toUpperCase()"/>
  </p>
  <p>
    <label for="tel" class=" m22">Teléfono:</label>
    <input name="tel" type="text" id="tel" size="10" maxlength="10"
        value="<?= esc(old('tel')) ?>" onkeyup="this.value=this.value.toUpperCase()" />
    <label for="cel" class="m50">Celular:</label>
    <input name="cel" type="text" id="cel" size="13" maxlength="13"
        value="<?= esc(old('cel')) ?>" onkeyup="this.value=this.value.toUpperCase()"/>
  </p>
  <p>
    <label for="email" class=" m39">E-mail:</label>
    <input name="email" type="text" id="email" size="40" maxlength="40" value="<?= esc(old('email')) ?>"/>
  </p>
  <p>
    <label for="direc" class="m19">Dirección:</label>
    <input name="direc" type="text" id="direc" size="105" maxlength="100"
        value="<?= esc(old('direc')) ?>" onkeyup="this.value=this.value.toUpperCase()"/>
  </p>
  <p>
    <label for="rfc" class="m50">RFC:</label>
    <input name="rfc" type="text" id="rfc" size="16" maxlength="13" required
        value="<?= esc(old('rfc')) ?>" onkeyup="this.value=this.value.toUpperCase()"/>
  </p>
  <p>
    <label for="comen">Comentarios:</label>
    <br />
    <textarea name="comen" id="comen" cols="45" rows="5" class="m85"
        onkeyup="this.value=this.value.toUpperCase()"><?= esc(old('comen')) ?></textarea>
  </p>
  <p align="center">
